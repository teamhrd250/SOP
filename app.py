...
with st.sidebar:
    ...
    page = st.radio(
        "NAVIGATION",
        ["Executive Flow", "Swimlane Flow", "Department Authority", "Process Detail", "Decision Gates", "RACI Matrix", "Presentation Mode"],
        index=1,
    )
    st.markdown("---")
    st.markdown("**LEGEND · DEPARTMENT**")
    for dep, cfg in DEPARTMENTS.items():
        st.markdown(f"<div style='margin:5px 0'><span style='display:inline-block;width:12px;height:12px;border-radius:3px;background:{cfg['color']};margin-right:8px'></span>{dep}</div>", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("**LEGEND · DECISION**")
    st.markdown("🟢 **GO / LOLOS**  ")
    st.markdown("🟠 **REVISE / HOLD**  ")
    st.markdown("🔴 **NO-GO / REJECT**  ")
    st.markdown("⚫ **LOST / CLOSE**  ")
...
def build_swimlane_html(processes, presentation=False):
    ...
    lane_x = {dep: 135 + i * 125 for i, dep in enumerate(deps)}
    gate_x, action_x, escalate_x = 1165, 1325, 1510
    row_h = 128
    top = 105
    width = 1635
    ...
    for dep in deps:
        ...
        lane_headers.append(
            f'<rect x="{x}" y="40" width="118" height="54" rx="4" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-width="1"/>'
            f'<text x="{x+59}" y="59" text-anchor="middle" class="hdr">{label.split(chr(10))[0]}</text>'
            + (f'<text x="{x+59}" y="73" text-anchor="middle" class="hdr">{label.split(chr(10))[1]}</text>' if "\n" in label else "")
        )
    ...
    for idx, p in enumerate(processes):
        ...
        # stage column
        rows.append(f'<rect x="10" y="{y}" width="115" height="108" rx="7" fill="#F8FBFF" stroke="#BFD0E2"/>')
        rows.append(f'<text x="62" y="{y+27}" text-anchor="middle" class="num">{p["id"]}</text>')
        rows.append(f'<text x="62" y="{y+49}" text-anchor="middle" class="stage">{p["stage"]}</text>')
        # process box
        box_w = 118
        rows.append(f'<rect class="process-box" data-id="{p["id"]}" x="{owner_x}" y="{y+14}" width="{box_w}" height="82" rx="8" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-width="1.3"/>')
        name_lines = split_lines(p["name"], 16)
        for li, line in enumerate(name_lines[:3]):
            rows.append(f'<text x="{owner_x+59}" y="{y+39+li*16}" text-anchor="middle" class="boxtext">{escape(line)}</text>')
        # Proses selalu menuju decision gate pada baris yang sama.
        # Tidak ada konektor langsung proses-ke-proses agar jalur GO tidak ganda.
        # flow to gate
        connectors.append(f'<path d="M {owner_x+118} {center_y} L {gate_x-50} {center_y}" class="flow-line" marker-end="url(#arrowBlue)"/>')
        # gate diamond
        rows.append(f'<polygon class="gate" data-id="{p["id"]}" points="{gate_x},{center_y-41} {gate_x+50},{center_y} {gate_x},{center_y+41} {gate_x-50},{center_y}" fill="#F6FAFF" stroke="#4E89C7" stroke-width="1.4"/>')
        gate_lines = split_lines(p["gate"], 14)
        for li, line in enumerate(gate_lines[:3]):
            rows.append(f'<text x="{gate_x}" y="{center_y-7+li*13}" text-anchor="middle" class="gateText">{escape(line)}</text>')
        # GO kembali ke swimlane pemilik proses berikutnya.
        # Jalur ini turun sedikit dari diamond, bergerak horizontal ke lane tujuan,
        # lalu masuk ke bagian atas kotak proses berikutnya.
        if idx < len(processes)-1:
            next_p = processes[idx+1]
            next_owner_x = lane_x[next_p["owner"]]
            next_y = top + (idx+1) * row_h
            next_center_x = next_owner_x + 59
            route_y = y + row_h - 10
            rows.append(f'<text x="{gate_x-4}" y="{center_y+62}" text-anchor="end" class="goLabel">GO</text>')
            go_path = (
                f'M {gate_x} {center_y+41} '
                f'L {gate_x} {route_y} '
                f'L {next_center_x} {route_y} '
                f'L {next_center_x} {next_y+14}'
            )
            connectors.append(f'<path d="{go_path}" class="go-line" marker-end="url(#arrowGreen)"/>')
        else:
            rows.append(f'<text x="{gate_x-4}" y="{center_y+62}" text-anchor="end" class="goLabel">GO / SELESAI</text>')
        # NO-GO action box
        connectors.append(f'<path d="M {gate_x+50} {center_y} L {action_x-8} {center_y}" class="no-line" marker-end="url(#arrowRed)"/>')
        rows.append(f'<text x="{gate_x+62}" y="{center_y-7}" class="noLabel">NO-GO</text>')
        rows.append(f'<rect class="action-box" data-id="{p["id"]}" x="{action_x}" y="{y+10}" width="170" height="90" rx="8" fill="#FFF2EE" stroke="#F06B59" stroke-width="1.2"/>')
        rows.append(f'<text x="{action_x+85}" y="{y+32}" text-anchor="middle" class="actionTitle">{escape(p["nogo"])}</text>')
        sol_lines = split_lines(p["solution"], 28)
        for li, line in enumerate(sol_lines[:3]):
            rows.append(f'<text x="{action_x+85}" y="{y+50+li*14}" text-anchor="middle" class="actionText">{escape(line)}</text>')
        # escalation
        connectors.append(f'<path d="M {action_x+170} {center_y} L {escalate_x-7} {center_y}" class="esc-line" marker-end="url(#arrowGray)"/>')
        rows.append(f'<rect x="{escalate_x}" y="{y+23}" width="110" height="64" rx="7" fill="#F5F8FC" stroke="#9FB1C5"/>')
        esc_lines = split_lines(p["escalation"], 18)
        for li, line in enumerate(esc_lines[:3]):
            rows.append(f'<text x="{escalate_x+55}" y="{y+46+li*14}" text-anchor="middle" class="escText">{escape(line)}</text>')

    grid_lines = []
    # column lines and rows
    for x in [5, 115] + [lane_x[d]-6 for d in deps] + [gate_x-50, action_x-10, escalate_x-10, width-5]:
        grid_lines.append(f'<line x1="{x}" y1="35" x2="{x}" y2="{height-20}" class="grid"/>')
    for i in range(len(processes)+1):
        y = top - 3 + i*row_h
        grid_lines.append(f'<line x1="5" y1="{y}" x2="{width-5}" y2="{y}" class="grid"/>')

    html = f"""
<!doctype html><html><head><meta charset='utf-8'/>
<style>
html,body{{margin:0;background:#F7FAFD;font-family:Inter,Arial,sans-serif;color:#0B243B;overflow:auto}}
.toolbar{{position:sticky;top:0;z-index:20;display:flex;gap:8px;align-items:center;padding:10px 12px;background:rgba(247,250,253,.96);border-bottom:1px solid #D9E4EF;backdrop-filter:blur(8px)}}
...
.toolbar .title{{font-weight:800;margin-right:auto;color:#071E33}}
#viewport{{padding:10px;overflow:auto;max-height:{'2100px' if presentation else '1850px'};background:white}}
...
.grid{{stroke:#E4EBF3;stroke-width:1}} .flow-line{{fill:none;stroke:#2D6596;stroke-width:1.2}} .go-line{{fill:none;stroke:#279647;stroke-width:1.5}} .no-line{{fill:none;stroke:#D94A3B;stroke-width:1.3}} .esc-line{{fill:none;stroke:#8798A9;stroke-width:1.1;stroke-dasharray:3 2}}
.hdr{{font-size:9px;font-weight:800;fill:#0B3152}} .num{{font-size:16px;font-weight:800;fill:#0B3152}} .stage{{font-size:7.3px;font-weight:800;fill:#2E5273}} .boxtext{{font-size:7.8px;font-weight:700;fill:#0A2F4E}} .gateText{{font-size:7.5px;font-weight:800;fill:#0B3152}} .goLabel{{font-size:8px;font-weight:900;fill:#21883D}} .noLabel{{font-size:7px;font-weight:900;fill:#D73D2D}} .actionTitle{{font-size:7.4px;font-weight:900;fill:#B52C23}} .actionText{{font-size:6.8px;font-weight:600;fill:#5B2A26}} .escText{{font-size:6.6px;font-weight:700;fill:#314B64}}
.process-box,.gate,.action-box{{cursor:pointer;transition:filter .15s,stroke-width .15s}} .process-box:hover,.gate:hover,.action-box:hover{{filter:drop-shadow(0 3px 5px rgba(0,0,0,.18));stroke-width:2.3}}
...
<div class='toolbar'>
 <span class='title'>Interactive Swimlane</span>
...
<defs>
 ...
 <marker id='arrowGray' markerWidth='8' markerHeight='8' refX='7' refY='4' orient='auto'><path d='M0,0 L8,4 L0,8 z' fill='#8798A9'/></marker>
</defs>
<rect x='5' y='35' width='{width-10}' height='{height-55}' rx='10' fill='white' stroke='#BFCFDF'/>
<rect x='10' y='40' width='118' height='54' rx='4' fill='#F1F6FB' stroke='#9CB2C7'/><text x='62' y='72' text-anchor='middle' class='hdr'>TAHAP</text>
{''.join(lane_headers)}
<rect x='{gate_x-45}' y='40' width='105' height='54' rx='4' fill='#EDF4FB' stroke='#4E89C7'/><text x='{gate_x}' y='68' text-anchor='middle' class='hdr'>DECISION GATE</text>
<rect x='{action_x}' y='40' width='170' height='54' rx='4' fill='#FFF2EE' stroke='#F06B59'/><text x='{action_x+85}' y='60' text-anchor='middle' class='hdr'>NO-GO: TINDAKAN</text><text x='{action_x+85}' y='73' text-anchor='middle' class='hdr'>& SOLUSI</text>
<rect x='{escalate_x}' y='40' width='110' height='54' rx='4' fill='#F2F5F8' stroke='#9FB1C5'/><text x='{escalate_x+55}' y='60' text-anchor='middle' class='hdr'>TUJUAN</text><text x='{escalate_x+55}' y='73' text-anchor='middle' class='hdr'>ESKALASI</text>
{''.join(grid_lines)}{''.join(connectors)}{''.join(rows)}
<rect x='390' y='{height-67}' width='400' height='45' rx='9' fill='#ECFAED' stroke='#4CAF50'/><text x='590' y='{height-43}' text-anchor='middle' style='font-size:13px;font-weight:900;fill:#207A36'>✓ CUSTOMER LOYAL & GROWTH</text><text x='590' y='{height-29}' text-anchor='middle' style='font-size:8px;font-weight:700;fill:#207A36'>Repeat Order / Renewal / Long-Term Partnership</text>
<rect x='1090' y='{height-67}' width='250' height='45' rx='9' fill='#FFF0EE' stroke='#EB5757'/><text x='1215' y='{height-42}' text-anchor='middle' style='font-size:13px;font-weight:900;fill:#B83030'>✕ LOST OPPORTUNITY</text><text x='1215' y='{height-28}' text-anchor='middle' style='font-size:8px;font-weight:700;fill:#B83030'>Close & record lost reason</text>
</svg></div></div>
...
function zoomIn(){{setZoom(zoom+.1)}} function zoomOut(){{setZoom(zoom-.1)}} function resetView(){{setZoom(1);document.getElementById('search').value='';document.getElementById('dept').value='ALL';applyFilters();document.getElementById('viewport').scrollTo(0,0)}}
...
if page == "Swimlane Flow":
    ...
    st.info("Gunakan tombol fullscreen pada toolbar diagram. Alur utama ditampilkan vertikal agar status Go/No-Go dan tindakan koreksi tetap terbaca.")
    html, h = build_swimlane_html(PROCESSES, presentation=True)
...
{p['risk']}""")
    ...
    st.markdown('<div class="section-title">Decision Gate Register</div>', unsafe_allow_html=True)
    for p in PROCESSES:
        with st.expander(f"Gate {p['id']} — {p['gate']}"):
            c1, c2, c3 = st.columns([1, 1.4, .8])
            c1.success(f"GO: {p['go']}")
            c2.error(f"NO-GO: {p['nogo']}\n\n{p['solution']}")
            c3.info(f"Eskalasi\n\n{p['escalation']}")

elif page == "RACI Matrix":
    st.markdown('<div class="section-title">RACI Matrix</div>', unsafe_allow_html=True)
    deps = list(DEPARTMENTS.keys())
    table = []
    for p in PROCESSES:
        row = {"Process": f"{p['id']}. {p['name']}"}
        for d in deps:
            row[d] = "A/R" if d == p["owner"] else "C" if d in infer_support(p) else "–"
        table.append(row)
    st.dataframe(table, use_container_width=True, hide_index=True)
    st.caption("A/R = Accountable & Responsible · C = Consulted · – = tidak terlibat langsung")
