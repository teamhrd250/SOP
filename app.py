    ...
    for idx, p in enumerate(processes):
        ...
        if p["id"] == 5:
            ...
            for item in PARALLEL_AUTHORITY:
                ...
                output_lines = split_lines(item["output"], 24)[:2]
                for li, line in enumerate(output_lines):
                    rows.append(f'<text x="{cx}" y="{parallel_y+52+li*12}" text-anchor="middle" class="parallelOutput">{escape(line)}</text>')
                rows.append(f'<text x="{cx}" y="{parallel_y+card_h-7}" text-anchor="middle" class="parallelHint">Klik detail ›</text>')
