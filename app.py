...
def build_swimlane_html(processes, presentation=False):
    ...
    for idx, p in enumerate(processes):
        ...
        # NO-GO box is placed close to the decision gate so the red line stays short.
        action_y = y + 103
        action_w = 250
        action_h = 108
        chart_right = left + stage_w + 8 + len(deps) * lane_w
        space_right = chart_right - (gate_cx + gate_rx)
        if space_right >= action_w + 34:
            action_x = gate_cx + gate_rx + 28
            branch_start_x = gate_cx + gate_rx
            branch_end_x = action_x - 8
            label_x = branch_start_x + 8
            label_anchor = "start"
        else:
            action_x = gate_cx - gate_rx - action_w - 28
            branch_start_x = gate_cx - gate_rx
            branch_end_x = action_x + action_w + 8
            label_x = branch_start_x - 8
            label_anchor = "end"

        rows.append(f'<text x="{label_x}" y="{gate_cy-9}" text-anchor="{label_anchor}" class="noLabel">NO-GO</text>')
        connectors.append(f'<path d="M {branch_start_x} {gate_cy} L {branch_end_x} {gate_cy}" class="no-line" marker-end="url(#arrowRed)"/>')
        rows.append(f'<rect class="action-box" data-id="{p["id"]}" x="{action_x}" y="{action_y}" width="{action_w}" height="{action_h}" rx="10" fill="#FFF2EE" stroke="#F06B59" stroke-width="1.5"/>')
        rows.append(f'<text x="{action_x+action_w/2}" y="{action_y+28}" text-anchor="middle" class="actionTitle">{escape(p["nogo"])}</text>')
        for li, line in enumerate(split_lines(p["solution"], 34)[:4]):
            rows.append(f'<text x="{action_x+action_w/2}" y="{action_y+52+li*16}" text-anchor="middle" class="actionText">{escape(line)}</text>')
        rows.append(f'<text x="{action_x+action_w/2}" y="{action_y+action_h+20}" text-anchor="middle" class="escText">Eskalasi: {escape(p["escalation"])}</text>')
...
#viewport{{padding:14px;overflow:auto;height:calc(100vh - 74px);min-height:760px;background:white}}
...
.hdr{{font-size:14px;font-weight:900;fill:#0B3152}} .num{{font-size:25px;font-weight:900;fill:#0B3152}} .stage{{font-size:11px;font-weight:900;fill:#2E5273}} .boxtext{{font-size:14px;font-weight:800;fill:#0A2F4E}} .gateText{{font-size:12px;font-weight:900;fill:#0B3152}} .goLabel{{font-size:11px;font-weight:900;fill:#21883D}} .noLabel{{font-size:11px;font-weight:900;fill:#D73D2D}} .actionTitle{{font-size:12px;font-weight:900;fill:#B52C23}} .actionText{{font-size:10.5px;font-weight:700;fill:#5B2A26}} .escText{{font-size:10px;font-weight:800;fill:#314B64}}
...
PHASES = [
    ...
    {"name":"Solution & Approval", "ids":[4,5,6], "owner":"PD + Management + Sales"},
    {"name":"Commercial", "ids":[7,8], "owner":"Sales + Legal + Finance"},
    {"name":"Delivery", "ids":[9,10,11], "owner":"Operation + Procurement"},
    {"name":"Acceptance & Cash", "ids":[12,13,14,15], "owner":"PD + Operation + Finance"},
...
if page == "Swimlane Flow":
    ...
    st.caption("Klik proses, decision gate, atau tindakan No-Go untuk membuka SOP, KPI, SLA, dokumen, PIC, risiko, dan eskalasi.")
