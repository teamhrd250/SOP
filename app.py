...
def build_swimlane_html(processes, presentation=False):
    ...
    if 2 in positions and 3 in positions and 4 in positions:
        ...
        if verify_centers:
            ...
            lead_verify_cards.append(
                f'<rect x="{merge_x_q-38}" y="{merge_mid_y-13}" width="76" height="26" rx="13" '
                f'fill="#EAF8EF" stroke="#258947" stroke-width="1.4"/>'
            )
            lead_verify_cards.append(
                f'<text x="{merge_x_q}" y="{merge_mid_y+3}" text-anchor="middle" class="bothGoText">ALL GO</text>'
            )
            lead_verify_connectors.append(
                f'<path d="M {merge_x_q} {merge_mid_y+13} L {merge_x_q} {solution_pos["cy"]} '
