...
def build_swimlane_html(processes, presentation=False):
    ...
    if 2 in positions and 3 in positions and 4 in positions:
        ...
        zone_top = min(lane_y["Legal"], lane_y["Procurement"], lane_y["Operation"]) + 8
        zone_bottom = max(lane_y["Legal"] + lane_h, lane_y["Procurement"] + lane_h, lane_y["Operation"] + lane_h) - 8
        zone_x = verify_x - 34
        zone_w = verify_card_w + 92
        lead_verify_cards.append(
            f'<rect x="{zone_x}" y="{zone_top}" width="{zone_w}" height="{zone_bottom-zone_top}" rx="12" class="parallelZone"/>'
        )
        lead_verify_cards.append(
            f'<text x="{zone_x+12}" y="{zone_top+16}" class="parallelZoneTitle">PARALLEL MANDATORY VERIFICATION</text>'
        )
        lead_verify_cards.append(
            f'<rect x="{split_x_q-31}" y="{zone_top+25}" width="62" height="22" rx="11" class="splitBadge"/>'
        )
        lead_verify_cards.append(
            f'<text x="{split_x_q}" y="{zone_top+40}" text-anchor="middle" class="zoneLabel">SPLIT</text>'
        )
        ...
        if verify_centers:
            ...
            gate_ry = 26
            lead_verify_cards.append(
                f'<rect x="{gate_cx-34}" y="{gate_cy-gate_ry-34}" width="68" height="20" rx="10" class="mergeBadge"/>'
            )
            lead_verify_cards.append(
                f'<text x="{gate_cx}" y="{gate_cy-gate_ry-20}" text-anchor="middle" class="zoneLabel">MERGE GATE</text>'
            )
            lead_verify_cards.append(
    ...
    internal_zone_top = min(lane_y["Sales"], lane_y["Product Development"], lane_y["Legal"], lane_y["Procurement"], lane_y["HSEQ"], lane_y["Operation"]) + 7
    internal_zone_bottom = max(
        lane_y["Sales"]+lane_h, lane_y["Product Development"]+lane_h, lane_y["Legal"]+lane_h,
        lane_y["Procurement"]+lane_h, lane_y["HSEQ"]+lane_h, lane_y["Operation"]+lane_h
    ) - 7
    support_cards.append(
        f'<rect x="{split_x-28}" y="{internal_zone_top}" width="{merge_x-split_x+56}" height="{internal_zone_bottom-internal_zone_top}" rx="12" class="parallelZone"/>'
    )
    support_cards.append(
        f'<text x="{split_x-16}" y="{internal_zone_top+17}" class="parallelZoneTitle">PARALLEL DEPARTMENT AUTHORITY</text>'
    )
    support_cards.append(
        f'<rect x="{split_x-31}" y="{internal_zone_top+27}" width="62" height="22" rx="11" class="splitBadge"/>'
    )
    support_cards.append(
        f'<text x="{split_x}" y="{internal_zone_top+42}" text-anchor="middle" class="zoneLabel">SPLIT</text>'
    )
    support_cards.append(
        f'<rect x="{merge_x-34}" y="{internal_zone_top+27}" width="68" height="22" rx="11" class="mergeBadge"/>'
    )
    support_cards.append(
        f'<text x="{merge_x}" y="{internal_zone_top+42}" text-anchor="middle" class="zoneLabel">MERGE</text>'
    )
    ...
    contract_zone_top = min(lane_y["Legal"], lane_y["Finance"]) + 8
    contract_zone_bottom = max(lane_y["Legal"]+lane_h, lane_y["Finance"]+lane_h) - 8
    support_cards.append(
        f'<rect x="{contract_split_x-28}" y="{contract_zone_top}" width="{contract_merge_x-contract_split_x+56}" height="{contract_zone_bottom-contract_zone_top}" rx="12" class="parallelZone"/>'
    )
    support_cards.append(
        f'<text x="{contract_split_x-16}" y="{contract_zone_top+17}" class="parallelZoneTitle">PARALLEL CONTRACT / PO REVIEW</text>'
    )
    support_cards.append(
        f'<rect x="{contract_split_x-31}" y="{contract_zone_top+27}" width="62" height="22" rx="11" class="splitBadge"/>'
    )
    support_cards.append(
        f'<text x="{contract_split_x}" y="{contract_zone_top+42}" text-anchor="middle" class="zoneLabel">SPLIT</text>'
    )
    support_cards.append(
        f'<rect x="{contract_merge_x-34}" y="{contract_zone_top+27}" width="68" height="22" rx="11" class="mergeBadge"/>'
    )
    support_cards.append(
        f'<text x="{contract_merge_x}" y="{contract_zone_top+42}" text-anchor="middle" class="zoneLabel">MERGE</text>'
    )
...
.zoneLabel{{font-size:8px;font-weight:900;fill:#355A78;letter-spacing:.06em}}
.splitBadge{{fill:#EDF5FC;stroke:#356C99;stroke-width:1.2}}
.mergeBadge{{fill:#EAF8EF;stroke:#258947;stroke-width:1.2}}
.parallelZone{{fill:#F7FAFD;stroke:#B9C9D8;stroke-width:1;stroke-dasharray:5 4}}
.parallelZoneTitle{{font-size:9px;font-weight:900;fill:#234A69;letter-spacing:.05em}}
...
<div class='toolbar'>
 ...
 <button onclick='resetView()'>Reset</button><button onclick='goToStart()'>⇤ Awal</button><button onclick='goToFinalProcess()'>Akhir ⇥</button><button id='edgeInfo' onclick='goToEnd()'>→ FULL KANAN</button><button onclick='fullscreen()'>⛶ Fullscreen</button><button onclick='window.print()'>Print / PDF</button><span style='font-size:11px;color:#49657D;margin-left:6px;white-space:nowrap'>SPLIT → PARALLEL WORK → MERGE GATE</span>
</div>
<div id='workspace'>
<div id='viewport'>
...
if page == "Swimlane Flow":
    ...
    st.info("Gunakan fullscreen pada toolbar. Baca area bercabang dengan pola SPLIT → PARALLEL WORK → MERGE GATE. Jalur hijau = GO, jalur merah = NO-GO/return.")
