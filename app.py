...
#workspace{{height:calc(100vh - 58px);min-height:1100px;overflow:hidden;background:#EAF0F5;position:relative}}
#viewport{{width:100%;height:100%;min-height:1100px;overflow:auto;background:#EAF0F5;position:relative;scrollbar-gutter:stable both-edges}}
...
#canvasWrap{{position:absolute;left:219px;top:14px;transform-origin:top left;will-change:auto}}
...
<div class='toolbar'>
 ...
 <button onclick='resetView()'>Reset</button><button onclick='goToStart()'>⇤ Awal</button><button onclick='goToEnd()'>Akhir ⇥</button><button id='edgeInfo' onclick='goToEnd()'>→ FULL KANAN</button><button onclick='fullscreen()'>⛶ Fullscreen</button><button onclick='window.print()'>Print / PDF</button>
</div>
<div id='workspace'>
<div id='viewport'>
...
function updateScrollSurface(){{
  ...
  const processW=Math.ceil(svg.width.baseVal.value*zoom);
  const processH=Math.ceil(svg.height.baseVal.value*zoom);
  const totalW=LEFT_W + 14 + processW + 120;
  const totalH=processH + 28;
  ...
  document.getElementById('canvasWrap').style.left=(LEFT_W+14)+'px';
  document.getElementById('canvasWrap').style.width=processW+'px';
  document.getElementById('canvasWrap').style.height=processH+'px';
}}
function setZoom(v){{
...
setTimeout(()=>{{setZoom(1);document.getElementById('viewport').scrollTo({{left:0,top:0}});}},150);
</script></body></html>
"""
    return html, height
