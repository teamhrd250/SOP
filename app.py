...
PROCESSES = [
    ...
    },
    {
        ...
        "activity": "Sales mengonsolidasikan hasil pekerjaan paralel dari Sales, Product Development, HSEQ, Procurement, dan Operation; melakukan review internal; lalu mengirim proposal final kepada customer.",
        ...
        "documents": "Quotation, proposal solution & product, dokumen HSEQ, timeline procurement, material readiness, proposal teknis, portfolio project, timeline project, manpower mapping, proposal managed service", "sla": "2 hari kerja",
    ...
    for idx, p in enumerate(processes):
        ...
        if p["id"] == 5:
            ...
            for item in PARALLEL_AUTHORITY:
                ...
                rows.append(f'<rect class="parallel-card" x="{x0}" y="{parallel_y}" width="{card_w}" height="{card_h}" rx="10" fill="{cfg2["soft"]}" stroke="{cfg2["color"]}" stroke-width="1.5"/>')
                title_lines = split_lines(item["title"], 22)[:2]
                for li, line in enumerate(title_lines):
                    rows.append(f'<text x="{cx}" y="{parallel_y+22+li*15}" text-anchor="middle" class="parallelTitle">{escape(line)}</text>')
                detail_lines = split_lines(item["detail"], 28)[:3]
                for li, line in enumerate(detail_lines):
                    rows.append(f'<text x="{cx}" y="{parallel_y+49+li*12}" text-anchor="middle" class="parallelText">{escape(line)}</text>')
...
PHASES = [
    ...
    {"name":"Solution & Approval", "ids":[4,5,6], "owner":"Management + Sales + PD + HSEQ + Procurement + Operation"},
