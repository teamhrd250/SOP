...
#workspace{{height:calc(100vh - 58px);display:flex;overflow:hidden;background:#EAF0F5;position:relative}}
#stickyPane{{flex:0 0 auto;width:205px;overflow:hidden;background:white;z-index:40;border-right:1px solid #B9C9D8;box-shadow:5px 0 12px rgba(16,42,65,.12)}}
#stickyScroll{{will-change:transform}}
#stickyScale{{transform-origin:top left;display:inline-block}}
#stickyPane svg{{display:block;background:white;border:0;box-shadow:none}}
#viewport{{flex:1 1 auto;height:100%;overflow:auto;background:#EAF0F5;position:relative}}
...
@media print{{.toolbar{{display:none}}#stickyPane{{display:none}}#viewport{{height:auto;overflow:visible}}#canvasWrap{{transform:none!important;padding:0}}html,body{{overflow:visible;background:white}}}}
</style></head><body>
<div class='toolbar'>
...
let zoom=1;
function setZoom(v){{zoom=Math.max(.25,Math.min(2.2,v));document.getElementById('canvasWrap').style.transform=`scale(${{zoom}})`;document.getElementById('stickyScale').style.transform=`scale(${{zoom}})`;document.getElementById('stickyPane').style.width=(${{205*zoom}})+'px';document.getElementById('zoomLabel').innerText=Math.round(zoom*100)+'%';syncStickyY();}}
function syncStickyY(){{const vp=document.getElementById('viewport');const ss=document.getElementById('stickyScroll');if(ss)ss.style.transform=`translateY(${{-vp.scrollTop}}px)`;}}
...
function fitWidth(){{const vp=document.getElementById('viewport');const svg=document.getElementById('chart');setZoom((vp.clientWidth-28)/((svg.width.baseVal.value-205)+28));vp.scrollTo(205*zoom,0);syncStickyY()}}
function fitHeight(){{const vp=document.getElementById('viewport');const svg=document.getElementById('chart');setZoom((vp.clientHeight-28)/(svg.height.baseVal.value+28));vp.scrollTo(205*zoom,0);syncStickyY()}}
function resetView(){{setZoom(1);document.getElementById('search').value='';document.getElementById('dept').value='ALL';applyFilters();document.getElementById('viewport').scrollTo(205,0);syncStickyY()}}
...
setTimeout(()=>{{fitHeight();syncStickyY();}},100);
</script></body></html>
"""
    return html, height
