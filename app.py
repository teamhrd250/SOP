...
def build_swimlane_html(processes, presentation=False):
    ...
    # Parallel responsibility indicators are shown as normal SOP support cards.
    # They stay within the corresponding department lane and align to the relevant stage.
    support_cards = []
    internal_x = col_x.get(5, left_w)
    for idx, item in enumerate(PARALLEL_AUTHORITY):
        dep = item["department"]
        y = lane_y[dep] + 78
        x = internal_x + col_w + 15 + (idx % 2) * 79
        w = 72
        cfg = DEPARTMENTS[dep]
        support_cards.append(f'<rect class="parallel-card" data-parallel="{idx}" x="{x}" y="{y}" width="{w}" height="35" rx="6" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-dasharray="3 2"/>')
        support_cards.append(f'<text x="{x+w/2}" y="{y+14}" text-anchor="middle" class="supportText">PARALLEL</text>')
        support_cards.append(f'<text x="{x+w/2}" y="{y+26}" text-anchor="middle" class="detailHint">KLIK</text>')

    # Legal and Finance contract/PO parallel cards aligned with stage 8.
    ...
    for idx, item in enumerate(CONTRACT_REVIEW_PARALLEL):
        ...
        x = contract_x + 64
        cfg = DEPARTMENTS[dep]
        support_cards.append(f'<rect class="contract-review-card" data-contract-review="{idx}" x="{x}" y="{y}" width="82" height="35" rx="6" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-dasharray="3 2"/>')
        support_cards.append(f'<text x="{x+41}" y="{y+14}" text-anchor="middle" class="supportText">PARALLEL</text>')
        support_cards.append(f'<text x="{x+41}" y="{y+26}" text-anchor="middle" class="detailHint">REVIEW</text>')
...
<script>
const processes={p_json}; const departments={dep_json}; const parallelAuthority={parallel_json}; const contractReviewParallel={contract_review_json};
let zoom=1;
