...
PROCESSES = [
    ...
    },
    {
        ...
        "gate": "Qualified?", "go": "Lanjut ke verifikasi Operation & Procurement",
        "nogo": "RETURN TO SALES", "solution": "Jika verifikasi Operation atau Procurement tidak memenuhi, hasil dikembalikan ke Sales untuk klarifikasi kebutuhan customer, penyesuaian scope, alternatif resource/perangkat, atau keputusan nurture/close opportunity.",
...
LEAD_QUALIFICATION_VERIFICATION = [
    {
        ...
        "nogo": "Kembalikan ke Sales bila manpower tidak tersedia, sertifikasi tidak sesuai, resource pendukung belum siap, atau requirement customer belum dapat dipenuhi."
    },
    {
        "department": "Procurement",
        ...
        "nogo": "Kembalikan ke Sales bila perangkat tidak available, lead time tidak memenuhi, spesifikasi tidak sesuai request, atau diperlukan alternatif supply."
    },
]
...
function showLeadVerificationDetail(index){{
  ...
  document.getElementById('modalBody').innerHTML=`<span class='badge' style='background:${{c.soft}};color:${{c.color}}'>${{item.department}}</span><h2>${{item.title}}</h2><div class='detailGrid'><div class='detail'><b>VERIFICATION SCOPE</b><span>${{item.detail}}</span></div><div class='detail'><b>OUTPUT</b><span>${{item.output}}</span></div><div class='detail'><b>JIKA GO</b><span>Hasil dikembalikan ke Sales untuk review dan konsolidasi. Setelah Sales menyatakan layak, proses dilanjutkan ke Need Assessment & Survey oleh Product Development.</span></div><div class='detail'><b>JIKA NO-GO</b><span>${{item.nogo}}</span></div></div>`; document.getElementById('modal').style.display='flex';
}}
