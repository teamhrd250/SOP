...
PROCESSES = [
    ...
    },
    {
        ...
        "gate": "BANT minimum terpenuhi?", "go": "Qualified opportunity; lanjut ke verifikasi paralel Operation, Procurement, dan Legal.",
    ...
    },
    {
        ...
        "nogo": "CLARIFY / RESURVEY", "solution": "Lengkapi requirement, lakukan klarifikasi bersama Sales/customer, atau lakukan survey ulang sampai data teknis cukup untuk menyusun solusi.",
...
def build_swimlane_html(processes, presentation=False):
    ...
    # Sales -> parallel Operation + Procurement + Legal -> ALL GO -> Product Development.
...
#workspace{{height:calc(100vh - 58px);display:flex;overflow:hidden;background:#EAF0F5;position:relative;min-width:0}}
#stickyPane{{flex:0 0 205px;width:205px;overflow:hidden;background:white;z-index:40;border-right:1px solid #B9C9D8;box-shadow:5px 0 12px rgba(16,42,65,.12)}}
#stickyScroll{{will-change:transform}}
#stickyScale{{transform-origin:top left;display:block}}
#stickyPane svg{{display:block;background:white;border:0;box-shadow:none}}
#viewport{{flex:1 1 0;min-width:0;height:100%;overflow-x:scroll;overflow-y:auto;background:#EAF0F5;position:relative;scrollbar-gutter:stable both-edges}}
#scrollSurface{{position:relative;min-width:100%;min-height:100%;}}
#canvasWrap{{position:absolute;left:14px;top:14px;transform-origin:top left;transition:transform .12s ease;will-change:transform}}
#viewport::-webkit-scrollbar{{height:15px;width:13px}}
...
<div class='toolbar'>
 ...
 <button onclick='resetView()'>Reset</button><button onclick='goToStart()'>⇤ Awal</button><button onclick='goToEnd()'>Akhir ⇥</button><button onclick='fullscreen()'>⛶ Fullscreen</button><button onclick='window.print()'>Print / PDF</button>
</div>
<div id='workspace'>
<div id='stickyPane'><div id='stickyScroll'><div id='stickyScale'>
...
function updateScrollSurface(){{
  ...
  const scaledW=Math.ceil(svg.width.baseVal.value*zoom)+PAD;
  const scaledH=Math.ceil(svg.height.baseVal.value*zoom)+PAD;
  surface.style.width=scaledW+'px';
  surface.style.height=scaledH+'px';
  surface.style.minWidth=scaledW+'px';
  surface.style.minHeight=scaledH+'px';
}}
function setZoom(v){{
  zoom=Math.max(.25,Math.min(2.2,v));
  document.getElementById('canvasWrap').style.transform=`scale(${{zoom}})`;
  document.getElementById('stickyScale').style.transform=`scale(${{zoom}})`;
  document.getElementById('stickyPane').style.flexBasis=(LEFT_W*zoom)+'px';
  document.getElementById('stickyPane').style.width=(LEFT_W*zoom)+'px';
  document.getElementById('zoomLabel').innerText=Math.round(zoom*100)+'%';
  updateScrollSurface();
  syncStickyY();
}}
function syncStickyY(){{
  const vp=document.getElementById('viewport');
  const ss=document.getElementById('stickyScroll');
  if(ss) ss.style.transform=`translateY(${{-vp.scrollTop}}px)`;
}}
document.getElementById('viewport').addEventListener('scroll',syncStickyY,{{passive:true}});
...
function fitWidth(){{
  const vp=document.getElementById('viewport');
  const svg=document.getElementById('chart');
  const processWidth=svg.width.baseVal.value-LEFT_W;
  setZoom(Math.max(.25,(vp.clientWidth-PAD)/processWidth));
  vp.scrollTo({{left:Math.round(LEFT_W*zoom),top:0,behavior:'auto'}});
  syncStickyY();
}}
function fitHeight(){{
  const vp=document.getElementById('viewport');
...
function resetView(){{
  ...
  applyFilters();
  document.getElementById('viewport').scrollTo({{left:LEFT_W,top:0,behavior:'auto'}});
  syncStickyY();
}}
function goToEnd(){{
  const vp=document.getElementById('viewport');
  vp.scrollTo({{left:vp.scrollWidth,top:vp.scrollTop,behavior:'smooth'}});
}}
function goToStart(){{
  const vp=document.getElementById('viewport');
  vp.scrollTo({{left:LEFT_W*zoom,top:vp.scrollTop,behavior:'smooth'}});
}}
...
setTimeout(()=>{{setZoom(1);document.getElementById('viewport').scrollTo({{left:LEFT_W,top:0}});syncStickyY();}},100);
</script></body></html>
"""
    return html, height
