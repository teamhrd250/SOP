...
def build_swimlane_html(processes, presentation=False):
    ...
    lane_x = {dep: 120 + i * 105 for i, dep in enumerate(deps)}
    gate_x, action_x, escalate_x = 1015, 1155, 1320
    row_h = 105
    top = 92
    width = 1435
    ...
    for dep in deps:
        ...
        lane_headers.append(
            f'<rect x="{x}" y="40" width="100" height="48" rx="4" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-width="1"/>'
            f'<text x="{x+50}" y="59" text-anchor="middle" class="hdr">{label.split(chr(10))[0]}</text>'
            + (f'<text x="{x+50}" y="73" text-anchor="middle" class="hdr">{label.split(chr(10))[1]}</text>' if "\n" in label else "")
        )
    ...
    for idx, p in enumerate(processes):
        ...
        # stage column
        rows.append(f'<rect x="10" y="{y}" width="100" height="90" rx="7" fill="#F8FBFF" stroke="#BFD0E2"/>')
        rows.append(f'<text x="60" y="{y+23}" text-anchor="middle" class="num">{p["id"]}</text>')
        rows.append(f'<text x="60" y="{y+42}" text-anchor="middle" class="stage">{p["stage"]}</text>')
        # process box
        box_w = 100
        rows.append(f'<rect class="process-box" data-id="{p["id"]}" x="{owner_x}" y="{y+12}" width="{box_w}" height="66" rx="8" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-width="1.3"/>')
        name_lines = split_lines(p["name"], 16)
        for li, line in enumerate(name_lines[:3]):
            rows.append(f'<text x="{owner_x+50}" y="{y+34+li*14}" text-anchor="middle" class="boxtext">{escape(line)}</text>')
        # connector previous to current
        if previous_center is not None:
            px, py = previous_center
            path = f'M {px} {py+33} L {px} {center_y-38} L {owner_x+50} {center_y-38} L {owner_x+50} {center_y-33}'
            connectors.append(f'<path d="{path}" class="flow-line" marker-end="url(#arrowBlue)"/>')
        previous_center = (owner_x+50, center_y)
        # flow to gate
        connectors.append(f'<path d="M {owner_x+100} {center_y} L {gate_x-42} {center_y}" class="flow-line" marker-end="url(#arrowBlue)"/>')
        # gate diamond
        rows.append(f'<polygon class="gate" data-id="{p["id"]}" points="{gate_x},{center_y-33} {gate_x+42},{center_y} {gate_x},{center_y+33} {gate_x-42},{center_y}" fill="#F6FAFF" stroke="#4E89C7" stroke-width="1.4"/>')
        ...
        # GO vertical indicator
        rows.append(f'<text x="{gate_x+3}" y="{center_y+48}" text-anchor="middle" class="goLabel">GO</text>')
        if idx < len(processes)-1:
            connectors.append(f'<path d="M {gate_x} {center_y+33} L {gate_x} {center_y+58}" class="go-line" marker-end="url(#arrowGreen)"/>')
        # NO-GO action box
        connectors.append(f'<path d="M {gate_x+42} {center_y} L {action_x-8} {center_y}" class="no-line" marker-end="url(#arrowRed)"/>')
        rows.append(f'<text x="{gate_x+62}" y="{center_y-7}" class="noLabel">NO-GO</text>')
        rows.append(f'<rect class="action-box" data-id="{p["id"]}" x="{action_x}" y="{y+9}" width="150" height="72" rx="8" fill="#FFF2EE" stroke="#F06B59" stroke-width="1.2"/>')
        rows.append(f'<text x="{action_x+75}" y="{y+29}" text-anchor="middle" class="actionTitle">{escape(p["nogo"])}</text>')
        sol_lines = split_lines(p["solution"], 28)
        for li, line in enumerate(sol_lines[:3]):
            rows.append(f'<text x="{action_x+75}" y="{y+44+li*12}" text-anchor="middle" class="actionText">{escape(line)}</text>')
        # escalation
        connectors.append(f'<path d="M {action_x+150} {center_y} L {escalate_x-7} {center_y}" class="esc-line" marker-end="url(#arrowGray)"/>')
        rows.append(f'<rect x="{escalate_x}" y="{y+20}" width="95" height="50" rx="7" fill="#F5F8FC" stroke="#9FB1C5"/>')
        esc_lines = split_lines(p["escalation"], 18)
        for li, line in enumerate(esc_lines[:3]):
            rows.append(f'<text x="{escalate_x+47}" y="{y+40+li*12}" text-anchor="middle" class="escText">{escape(line)}</text>')

    grid_lines = []
    # column lines and rows
    for x in [5, 115] + [lane_x[d]-5 for d in deps] + [gate_x-50, action_x-10, escalate_x-10, width-5]:
...
#viewport{{padding:10px;overflow:auto;max-height:{'1600px' if presentation else '1450px'};background:white}}
...
<rect x='10' y='40' width='100' height='48' rx='4' fill='#F1F6FB' stroke='#9CB2C7'/><text x='60' y='68' text-anchor='middle' class='hdr'>TAHAP</text>
{''.join(lane_headers)}
<rect x='{gate_x-45}' y='40' width='90' height='48' rx='4' fill='#EDF4FB' stroke='#4E89C7'/><text x='{gate_x}' y='68' text-anchor='middle' class='hdr'>DECISION GATE</text>
<rect x='{action_x}' y='40' width='150' height='48' rx='4' fill='#FFF2EE' stroke='#F06B59'/><text x='{action_x+75}' y='60' text-anchor='middle' class='hdr'>NO-GO: TINDAKAN</text><text x='{action_x+75}' y='73' text-anchor='middle' class='hdr'>& SOLUSI</text>
<rect x='{escalate_x}' y='40' width='95' height='48' rx='4' fill='#F2F5F8' stroke='#9FB1C5'/><text x='{escalate_x+47}' y='60' text-anchor='middle' class='hdr'>TUJUAN</text><text x='{escalate_x+47}' y='73' text-anchor='middle' class='hdr'>ESKALASI</text>
...
setZoom({'.78' if presentation else '.82'});
</script></body></html>
"""
    return html, height
...
if page == "Swimlane Flow":
    ...
    st.caption("Klik kotak proses, decision gate, atau tindakan No-Go untuk membuka detail. Gunakan search, filter, zoom, fullscreen, dan Print/PDF pada toolbar diagram.")
