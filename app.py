...
PROCESSES = [
    {
        ...
        "nogo": "REJECT / CLOSE LEAD", "solution": "Lead tidak sesuai target pasar, data tidak valid, atau tidak memiliki kebutuhan yang relevan. Catat alasan penolakan di CRM dan tutup lead.",
    ...
    },
    {
        ...
        "gate": "BANT minimum terpenuhi?", "go": "Qualified opportunity; lakukan verifikasi paralel oleh Operation dan Procurement sebelum masuk ke Product Development.",
        "nogo": "NURTURE / REQUALIFY / CLOSE", "solution": "Jika budget, authority, need, atau timeline belum memadai, kembalikan ke Sales untuk klarifikasi/re-kualifikasi. Masukkan ke nurture list atau close opportunity bila tidak layak dilanjutkan.",
        "escalation": "Sales Manager", "output": "Qualified opportunity yang siap diverifikasi lintas fungsi",
        "documents": "Qualification checklist, CRM opportunity, preliminary requirement, manpower/certification check, device availability check", "sla": "2–3 hari kerja",
        "kpi": "Lead-to-opportunity conversion dan verification turnaround time", "risk": "Opportunity semu, resource tidak tersedia, sertifikasi tidak memenuhi, perangkat tidak available, atau request tidak sesuai kemampuan",
    },
    {
        ...
        "gate": "Requirement dan data survey sudah jelas serta lengkap?", "go": "Requirement tervalidasi; lanjut ke Solution Design oleh Product Development.",
        "nogo": "CLARIFY / RESURVEY", "solution": "Lengkapi requirement, lakukan klarifikasi bersama Sales/customer, atau lakukan survey ulang sampai data teknis memadai untuk desain solusi.",
    ...
    },
    {
        ...
        "gate": "Teknis, komersial, legal, supply, HSEQ, resource, jadwal, dan risiko disetujui?", "go": "Internal Review disetujui; distribusikan pekerjaan secara paralel ke Sales, Product Development, HSEQ, Legal, Procurement, dan Operation.",
        "nogo": "REVISE / HOLD / REJECT", "solution": "Kembalikan temuan ke departemen terkait dengan catatan revisi, PIC, target waktu, dan approval ulang. Hold bila menunggu informasi/approval; reject bila risiko tidak dapat diterima.",
    ...
    },
    {
        ...
        "nogo": "REVISE / RE-SUBMIT / WITHDRAW", "solution": "Konsolidasikan feedback customer, revisi harga/scope/term/solusi sesuai batas kewenangan, lalu submit ulang. Withdraw bila persyaratan customer tidak dapat dipenuhi.",
    ...
    },
    {
        ...
        "nogo": "DEADLOCK / LOST", "solution": "Lakukan final offer atau eskalasi sesuai kewenangan. Jika tidak tercapai kesepakatan, dokumentasikan lost reason dan tutup opportunity.",
    ...
    },
    {
        ...
        "gate": "Legal dan Finance keduanya memberikan clearance?", "go": "BOTH GO: hasil review Legal dan Finance dinyatakan clear; lanjut ke Kick Off Internal.",
        "nogo": "REVISE CONTRACT / PO / ESCALATE", "solution": "Temuan Legal atau Finance dikembalikan ke Sales/customer untuk revisi kontrak, PO, SLA, payment term, tax, invoice requirement, atau approval risiko Direksi. Proses tidak boleh lanjut sebelum kedua fungsi clear.",
    ...
    },
    {
        ...
        "gate": "Project siap dieksekusi?", "go": "Handover lengkap, PM/tim, scope, WBS, jadwal, budget, dan risiko telah disepakati; lanjut ke Procurement & Logistics.",
    ...
    },
    {
        ...
        "nogo": "REPAIR / RECTIFY / RETEST", "solution": "Lakukan troubleshooting dan corrective action pada temuan, update konfigurasi/instalasi bila perlu, kemudian retest sampai acceptance criteria terpenuhi.",
...
PARALLEL_AUTHORITY = [
    {
        ...
        "nogo": "Kembalikan untuk revisi bila harga, margin, term, SLA, warranty, atau payment term belum sesuai approval."
    },
    {
    ...
    },
    {
        ...
        "go": "Legal requirement project teridentifikasi dan dokumen yang diperlukan tersedia/terverifikasi; serahkan checklist ke Sales.",
...
CONTRACT_REVIEW_PARALLEL = [
    {
        ...
        "go": "Legal memberikan clearance atas kontrak/SLA/NDA/liability/warranty/acceptance dan legal requirement.",
...
LEAD_QUALIFICATION_VERIFICATION = [
    {
        ...
        "nogo": "Kembalikan ke Sales untuk klarifikasi/revisi bila manpower tidak tersedia, sertifikasi tidak sesuai, resource pendukung belum siap, akses/site bermasalah, atau requirement belum dapat dipenuhi."
    },
    {
        "department": "Procurement",
        ...
        "nogo": "Kembalikan ke Sales untuk klarifikasi/revisi bila perangkat tidak available, lead time tidak memenuhi, spesifikasi tidak sesuai request, atau alternatif supply belum dapat disepakati."
    },
]
...
def build_swimlane_html(processes, presentation=False):
    ...
    # Lead Qualification cross-functional verification: Operation + Procurement in parallel.
    lead_verify_cards = []
    ...
    if 2 in positions and 3 in positions:
        ...
        merge_x_q = verify_x + verify_card_w + 12
        verify_centers = []
        for vidx, item in enumerate(LEAD_QUALIFICATION_VERIFICATION):
            ...
            lead_verify_cards.append(f'<rect class="lead-verify-card process-card" data-lead-verify="{vidx}" x="{verify_x}" y="{vy}" width="{verify_card_w}" height="{verify_card_h}" rx="8" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-width="1.8"/>')
            lead_verify_cards.append(f'<rect x="{verify_x}" y="{vy}" width="7" height="{verify_card_h}" rx="4" fill="{cfg["color"]}" pointer-events="none"/>')
            lead_verify_cards.append(f'<text x="{vcx+3}" y="{vy+15}" text-anchor="middle" class="parallelDeptText">{escape(dep)}</text>')
            for li,line in enumerate(split_lines(item["title"],23)[:3]):
                lead_verify_cards.append(f'<text x="{vcx+3}" y="{vy+31+li*14}" text-anchor="middle" class="parallelProcessText">{escape(line)}</text>')
            lead_verify_cards.append(f'<text x="{vcx+3}" y="{vy+verify_card_h-7}" text-anchor="middle" class="parallelClickText">Klik detail</text>')
            lead_verify_connectors.append(f'<path d="M {split_x_q} {vcy} L {verify_x} {vcy}" class="parallelBranch" marker-end="url(#arrowBlue)"/>')
            lead_verify_connectors.append(f'<path d="M {verify_x+verify_card_w} {vcy} L {merge_x_q} {vcy}" class="parallelReturn"/>')
        if verify_centers:
            min_vy=min(cy for _,_,cy in verify_centers); max_vy=max(cy for _,_,cy in verify_centers)
            qual_go_y=qual_pos["y"]+105
            lead_verify_connectors.append(f'<text x="{qual_pos["cx"]+8}" y="{qual_go_y-6}" class="goText">GO · VERIFY</text>')
            lead_verify_connectors.append(f'<path d="M {qual_pos["cx"]} {qual_go_y} L {split_x_q} {qual_go_y}" class="parallelGoMain" marker-end="url(#arrowGreen)"/>')
            lead_verify_connectors.append(f'<path d="M {split_x_q} {min(min_vy,qual_go_y)} L {split_x_q} {max(max_vy,qual_go_y)}" class="parallelSpine"/>')
            lead_verify_connectors.append(f'<path d="M {merge_x_q} {min_vy} L {merge_x_q} {max_vy}" class="parallelMergeSpine"/>')
            sales_y=lane_y["Sales"]+24; sales_card_x=verify_x+verify_card_w+28; sales_card_w=160; sales_card_h=66
            sales_cx=sales_card_x+sales_card_w/2; sales_cy=sales_y+sales_card_h/2
            lead_verify_cards.append(f'<rect class="lead-sales-review-card" x="{sales_card_x}" y="{sales_y}" width="{sales_card_w}" height="{sales_card_h}" rx="8" fill="{DEPARTMENTS["Sales"]["soft"]}" stroke="{DEPARTMENTS["Sales"]["color"]}" stroke-width="1.8"/>')
            lead_verify_cards.append(f'<text x="{sales_cx}" y="{sales_y+20}" text-anchor="middle" class="parallelDeptText">Sales</text>')
            lead_verify_cards.append(f'<text x="{sales_cx}" y="{sales_y+38}" text-anchor="middle" class="parallelProcessText">Verification Review</text>')
            lead_verify_cards.append(f'<text x="{sales_cx}" y="{sales_y+52}" text-anchor="middle" class="parallelProcessText">& Consolidation</text>')
            lead_verify_connectors.append(f'<path d="M {merge_x_q} {sales_cy} L {sales_card_x} {sales_cy}" class="parallelReturn" marker-end="url(#arrowGreen)"/>')
            lead_verify_connectors.append(f'<path d="M {sales_card_x+sales_card_w} {sales_cy} L {need_pos["x"]-14} {sales_cy} L {need_pos["x"]-14} {need_pos["cy"]} L {need_pos["x"]} {need_pos["cy"]}" class="parallelReturn" marker-end="url(#arrowGreen)"/>')
            nogo_y=sales_y+sales_card_h+14
            lead_verify_cards.append(f'<text x="{sales_cx}" y="{nogo_y-5}" text-anchor="middle" class="noGoReturnText">NO-GO · RETURN TO SALES</text>')
            for dep,vcx,vcy in verify_centers:
                lead_verify_connectors.append(f'<path d="M {merge_x_q-4} {vcy+10} L {merge_x_q+8} {vcy+10} L {merge_x_q+8} {nogo_y} L {sales_cx} {nogo_y} L {sales_cx} {sales_y+sales_card_h}" class="noFlow" marker-end="url(#arrowRed)"/>')
...
@media print{{.toolbar{{display:none}}#viewport{{height:auto;overflow:visible}}#canvasWrap{{transform:none!important;padding:0}}html,body{{overflow:visible;background:white}}}}
</style></head><body>
<div class='toolbar'>
...
let zoom=1;
function setZoom(v){{zoom=Math.max(.25,Math.min(2.2,v));document.getElementById('canvasWrap').style.transform=`scale(${{zoom}})`;document.getElementById('zoomLabel').innerText=Math.round(zoom*100)+'%';}}
...
function showLeadVerificationDetail(index){{
  ...
  document.getElementById('modalBody').innerHTML=`<span class='badge' style='background:${{c.soft}};color:${{c.color}}'>${{item.department}}</span><h2>${{item.title}}</h2><div class='detailGrid'><div class='detail'><b>VERIFICATION SCOPE</b><span>${{item.detail}}</span></div><div class='detail'><b>OUTPUT</b><span>${{item.output}}</span></div><div class='detail'><b>JIKA GO</b><span>Jika Operation dan Procurement keduanya GO, proses dapat dilanjutkan ke Need Assessment & Survey oleh Product Development. Jika salah satu NO-GO, requirement dikembalikan ke Sales untuk klarifikasi atau revisi.</span></div><div class='detail'><b>JIKA NO-GO</b><span>${{item.nogo}}</span></div></div>`; document.getElementById('modal').style.display='flex';
}}
...
def infer_support(p):
    mapping = {
        1: [], 2: ["Operation", "Procurement"], 3: ["Sales", "Operation"], 4: ["Sales", "Operation", "Procurement"],
...
# -----------------------------
PHASES = [
    {"name":"Opportunity", "ids":[1,2,3], "owner":"Sales + Operation + Procurement + Product Development"},
