...
#viewport{{height:calc(100vh - 58px);overflow:auto;background:#EAF0F5;position:relative}}\n#stickyLeft{{position:absolute;left:14px;top:14px;z-index:35;width:{left_w}px;height:{height}px;overflow:hidden;pointer-events:none;background:white}}\n#stickyLeftWrap{{transform-origin:top left;display:inline-block;transition:transform .12s ease}}\n#stickyLeft svg{{display:block;background:white;border:0;box-shadow:6px 0 12px rgba(16,42,65,.12)}}
...
function syncStickyLeft(){{const vp=document.getElementById('viewport');const sl=document.getElementById('stickyLeft');if(sl) sl.style.transform=`translateX(${{vp.scrollLeft}}px)`;}}
document.getElementById('viewport').addEventListener('scroll',syncStickyLeft,{{passive:true}});
function zoomIn(){{setZoom(zoom+.1)}} function zoomOut(){{setZoom(zoom-.1)}} function actualSize(){{setZoom(1)}}
function fitWidth(){{const vp=document.getElementById('viewport');const svg=document.getElementById('chart');setZoom((vp.clientWidth-28)/(svg.width.baseVal.value+28));vp.scrollTo(0,0);syncStickyLeft()}}
function fitHeight(){{const vp=document.getElementById('viewport');const svg=document.getElementById('chart');setZoom((vp.clientHeight-28)/(svg.height.baseVal.value+28));vp.scrollTo(0,0);syncStickyLeft()}}
function resetView(){{setZoom(1);document.getElementById('search').value='';document.getElementById('dept').value='ALL';applyFilters();document.getElementById('viewport').scrollTo(0,0);syncStickyLeft()}}
...
setTimeout(()=>{{fitHeight();syncStickyLeft();}},100);
</script></body></html>
"""
    return html, height
