...
def build_swimlane_html(processes, presentation=False):
    ...
    # Lead Qualification cross-functional verification: Procurement + Operation in parallel.
    # GO from Lead Qualification MUST enter both verification lanes first.
    # Only after BOTH return GO does the merged flow continue directly to
    # Need Assessment & Survey in Product Development. NO-GO returns to Sales.
    lead_verify_cards = []
    ...
    if 2 in positions and 3 in positions:
        ...
        # Put verification cards in the next process column so the sequence is visually explicit.
        verify_x = col_x.get(3, left_w) + 18
        verify_card_w = 166
        verify_card_h = 72
        split_x_q = verify_x - 18
        merge_x_q = verify_x + verify_card_w + 18
        verify_centers = []

        # Render Procurement first visually, then Operation; each remains in its own lane.
        ordered_verification = sorted(
            enumerate(LEAD_QUALIFICATION_VERIFICATION),
            key=lambda pair: 0 if pair[1]["department"] == "Procurement" else 1
        )
        for vidx, item in ordered_verification:
            dep = item["department"]
            cfg = DEPARTMENTS[dep]
        ...
        if verify_centers:
            ...
            # Main green GO line from Lead Qualification to the split spine.
            ...
            # One blue distribution spine branches into Procurement and Operation.
            spine_min = min(min_vy, qual_go_y)
            spine_max = max(max_vy, qual_go_y)
            lead_verify_connectors.append(
                f'<path d="M {split_x_q} {spine_min} L {split_x_q} {spine_max}" class="parallelSpine"/>'
            )
            ...
            # Both GO outputs enter one green merge spine.
            lead_verify_connectors.append(
                f'<path d="M {merge_x_q} {min_vy} L {merge_x_q} {max_vy}" class="parallelMergeSpine"/>'
            )
            for dep, vcx, vcy in verify_centers:
                lead_verify_connectors.append(
                    f'<path d="M {verify_x+verify_card_w} {vcy} L {merge_x_q} {vcy}" class="parallelReturn"/>'
                )

            # BOTH GO -> directly to Product Development / Need Assessment & Survey.
            merge_mid_y = (min_vy + max_vy) / 2
            lead_verify_connectors.append(
                f'<text x="{merge_x_q+7}" y="{merge_mid_y-7}" class="goText">BOTH GO</text>'
            )
            lead_verify_connectors.append(
                f'<path d="M {merge_x_q} {merge_mid_y} L {merge_x_q+18} {merge_mid_y} '
                f'L {merge_x_q+18} {need_pos["cy"]} L {need_pos["x"]} {need_pos["cy"]}" '
                f'class="parallelReturn" marker-end="url(#arrowGreen)"/>'
            )

            # NO-GO from either verification returns to Sales for clarification/revision.
            sales_return_x = qual_pos["x"] + qual_pos["w"] + 10
            sales_return_y = lane_y["Sales"] + lane_h - 18
            for dep, vcx, vcy in verify_centers:
                lead_verify_connectors.append(
                    f'<path d="M {verify_x+verify_card_w} {vcy+11} L {merge_x_q+10} {vcy+11} '
                    f'L {merge_x_q+10} {sales_return_y} L {sales_return_x} {sales_return_y}" '
                    f'class="noFlow" marker-end="url(#arrowRed)"/>'
                )
            lead_verify_cards.append(
                f'<text x="{sales_return_x+8}" y="{sales_return_y-6}" class="noGoReturnText">NO-GO → SALES: KLARIFIKASI / REVISI REQUIREMENT</text>'
            )
