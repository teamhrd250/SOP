...
# -----------------------------
DEPARTMENTS = {
    "Sales": {"color": "#2F80ED", "soft": "#0B1B2D", "icon": "◆"},
    "Product Development": {"color": "#7B61FF", "soft": "#1A1233", "icon": "◇"},
    "Finance": {"color": "#27AE60", "soft": "#0D2115", "icon": "●"},
    "Legal": {"color": "#8E5CC7", "soft": "#1E1428", "icon": "■"},
    "Procurement": {"color": "#F2994A", "soft": "#2A190B", "icon": "▲"},
    "HSEQ": {"color": "#17A2B8", "soft": "#0B2227", "icon": "✚"},
    "Operation": {"color": "#25A8B8", "soft": "#0B2227", "icon": "✦"},
    "After Sales": {"color": "#EB5757", "soft": "#2A1114", "icon": "●"},
    "Management": {"color": "#D6A11D", "soft": "#2A210B", "icon": "◆"},
}
...
st.markdown(
    """
<style>
:root { --navy:#071E33; --navy2:#0B3152; --blue:#1477C9; --bg:#05070A; }
[data-testid="stAppViewContainer"] { background: #05070A; color:#EAF3FA; }
...
.metric-card { background:#101722; border:1px solid #26384A; border-radius:14px; padding:15px 17px; box-shadow:0 5px 20px rgba(0,0,0,.28); }
.metric-card b { font-size:26px; color:#F5FAFF; }
.metric-card span { display:block; color:#AFC3D5; font-size:11px; letter-spacing:.08em; margin-top:3px; }
.section-title { font-size:22px; font-weight:800; color:#F5FAFF; margin:12px 0 8px; }
...
def build_swimlane_html(processes, presentation=False):
    ...
    contract_parallel_h = 190
    height = top + len(processes) * row_h + extra_parallel_h + contract_parallel_h + 90

    def row_y(index):
        offset = 0
        if index > 4:
            offset += extra_parallel_h
        if index > 7:
            offset += contract_parallel_h
        return top + index * row_h + offset
    ...
    for idx, p in enumerate(processes):
        ...
        # Stage column
        rows.append(f'<rect x="{left}" y="{y}" width="{stage_w-8}" height="{row_h-12}" rx="8" fill="#0B1118" stroke="#30475A"/>')
        ...
        # Process box
        rows.append(f'<rect id="process-{p["id"]}" class="process-box" data-id="{p["id"]}" x="{box_x}" y="{box_y}" width="{box_w}" height="{box_h}" rx="9" fill="{cfg["soft"]}" stroke="{cfg["color"]}" stroke-width="1.5"/>')
        ...
        rows.append(f'<polygon class="gate" data-id="{p["id"]}" points="{gate_cx},{gate_cy-gate_ry} {gate_cx+gate_rx},{gate_cy} {gate_cx},{gate_cy+gate_ry} {gate_cx-gate_rx},{gate_cy}" fill="#0C1420" stroke="#4EA3FF" stroke-width="1.5"/>')
        ...
        if p["id"] == 5:
            parallel_y = y + 268
            rows.append(f'<rect x="{lane_x["Sales"]-8}" y="{parallel_y-38}" width="{lane_x["Operation"]+lane_w-lane_x["Sales"]+16}" height="150" rx="12" fill="#080C12" stroke="#4EA3FF" stroke-width="1.2" stroke-dasharray="5 4"/>')
        ...
        # Special Legal + Finance parallel review after Contract / PO process (process 8).
        elif p["id"] == 8:
            contract_y = y + 250
            group_left = lane_x["Finance"] - 10
            group_right = lane_x["Legal"] + lane_w + 10
            group_w = group_right - group_left
            rows.append(f'<rect x="{group_left}" y="{contract_y-38}" width="{group_w}" height="132" rx="12" fill="#080C12" stroke="#4EA3FF" stroke-width="1.2" stroke-dasharray="5 4"/>')
            rows.append(f'<text x="{group_left+group_w/2}" y="{contract_y-18}" text-anchor="middle" class="parallelGroupTitle">LEGAL + FINANCE PARALLEL CONTRACT / PO REVIEW</text>')

            start_y = gate_cy + gate_ry
            split_y = contract_y - 18
            join_y = y + row_h + contract_parallel_h - 22
            rows.append(f'<text x="{gate_cx}" y="{start_y+18}" text-anchor="middle" class="goLabel">GO · PARALLEL</text>')
            connectors.append(f'<path d="M {gate_cx} {start_y} L {gate_cx} {split_y}" class="go-line" marker-end="url(#arrowGreen)"/>')

            contract_centers = []
            for cidx, item in enumerate(CONTRACT_REVIEW_PARALLEL):
                dep = item["department"]
                card_w2 = 176
                card_h2 = 82
                x0 = lane_x[dep] + 18
                cx = x0 + card_w2/2
                contract_centers.append(cx)
                cfg2 = DEPARTMENTS[dep]
                rows.append(f'<rect class="contract-review-card" data-contract-review="{cidx}" x="{x0}" y="{contract_y}" width="{card_w2}" height="{card_h2}" rx="10" fill="{cfg2["soft"]}" stroke="{cfg2["color"]}" stroke-width="1.5"/>')
                rows.append(f'<text x="{cx}" y="{contract_y+18}" text-anchor="middle" class="parallelDept">{escape(dep)}</text>')
                for li, line in enumerate(split_lines(item["title"], 21)[:3]):
                    rows.append(f'<text x="{cx}" y="{contract_y+39+li*14}" text-anchor="middle" class="parallelTitle">{escape(line)}</text>')
                rows.append(f'<text x="{cx}" y="{contract_y+card_h2-8}" text-anchor="middle" class="parallelHint">Klik untuk detail ›</text>')
                connectors.append(f'<path d="M {gate_cx} {split_y} L {cx} {split_y} L {cx} {contract_y}" class="parallel-line" marker-end="url(#arrowBlue)"/>')
                connectors.append(f'<path d="M {cx} {contract_y+card_h2} L {cx} {join_y}" class="parallel-return"/>')

            min_cc, max_cc = min(contract_centers), max(contract_centers)
            connectors.append(f'<path d="M {min_cc} {join_y} L {max_cc} {join_y}" class="parallel-return"/>')

            merge_x = (min_cc + max_cc) / 2
            merge_y = join_y + 18
            rows.append(f'<polygon points="{merge_x},{merge_y-18} {merge_x+26},{merge_y} {merge_x},{merge_y+18} {merge_x-26},{merge_y}" fill="#0A1B10" stroke="#37D66B" stroke-width="1.4"/>')
            rows.append(f'<text x="{merge_x}" y="{merge_y+4}" text-anchor="middle" class="mergeText">MERGE</text>')
            connectors.append(f'<path d="M {merge_x} {join_y} L {merge_x} {merge_y-18}" class="parallel-return"/>')

            nxt = processes[idx+1]
            next_x = lane_x[nxt["owner"]] + lane_w/2 - 4
            next_y = row_y(idx+1) + 12
            connectors.append(f'<path d="M {merge_x} {merge_y+18} L {merge_x} {merge_y+34} L {next_x} {merge_y+34} L {next_x} {next_y}" class="go-line" marker-end="url(#arrowGreen)"/>')
        ...
        rows.append(f'<rect class="action-box" data-id="{p["id"]}" x="{action_x}" y="{action_y}" width="{action_w}" height="{action_h}" rx="9" fill="#241012" stroke="#FF4D4F" stroke-width="1.5"/>')
    ...
    for i in range(len(processes)+1):
        ...
        if i > 5:
            yy += extra_parallel_h
        if i > 8:
            yy += contract_parallel_h
        grid_lines.append(f'<line x1="{left}" y1="{yy}" x2="{width-8}" y2="{yy}" class="grid"/>')

    html = f"""
<!doctype html><html><head><meta charset='utf-8'/>
<style>
html,body{{margin:0;background:#05070A;font-family:Inter,Arial,sans-serif;color:#EAF3FA;overflow:auto}}
.toolbar{{position:sticky;top:0;z-index:20;display:flex;flex-wrap:wrap;gap:10px;align-items:center;padding:12px 14px;background:rgba(5,7,10,.98);border-bottom:1px solid #263646;backdrop-filter:blur(8px)}}
.toolbar button,.toolbar select,.toolbar input{{border:1px solid #32485C;background:#0E151D;border-radius:9px;padding:10px 12px;font-size:14px;color:#EAF3FA}}
.toolbar button{{cursor:pointer;font-weight:800}} .toolbar .title{{font-weight:900;font-size:18px;margin-right:auto;color:#F5FAFF}}
#viewport{{padding:14px 330px 14px 14px;overflow:auto;height:calc(100vh - 74px);min-height:760px;background:#05070A;position:relative;scroll-behavior:smooth}}
.mode-group{{display:flex;gap:6px;align-items:center;padding:4px;border:1px solid #2D4154;border-radius:10px;background:#0A1118}}
.mode-btn{{border:0!important;background:transparent!important;padding:7px 10px!important}}
.mode-btn.active{{background:#1477C9!important;color:white!important}}
.phase-nav{{display:flex;gap:6px;flex-wrap:wrap;width:100%;padding-top:2px}}
.phase-nav button{{font-size:12px!important;padding:7px 10px!important}}
#sidePanel{{position:fixed;right:14px;top:88px;width:300px;max-height:calc(100vh - 108px);overflow:auto;z-index:34;background:#0D151E;border:1px solid #2D4154;border-radius:14px;padding:15px;color:#EAF3FA;box-shadow:0 12px 30px rgba(0,0,0,.38)}}
#sidePanel h3{{margin:0 0 4px;font-size:18px;color:#F5FAFF}} #sidePanel .muted{{color:#92A9BC;font-size:12px}}
.panel-section{{margin-top:13px;padding-top:11px;border-top:1px solid #263746}} .panel-section b{{display:block;font-size:10px;letter-spacing:.08em;color:#7FA9C9;margin-bottom:5px}} .panel-section span{{font-size:13px;line-height:1.45}}
#miniMap{{height:170px;overflow:auto;border:1px solid #263746;border-radius:10px;padding:8px;background:#070B10;margin-top:10px}}
.mini-item{{display:flex;gap:7px;align-items:center;padding:6px;border-radius:7px;cursor:pointer;font-size:11px;color:#BFD0DE}}
.mini-item:hover,.mini-item.active{{background:#13283A;color:white}} .mini-dot{{width:8px;height:8px;border-radius:50%;flex:0 0 auto}}
.focused{{filter:drop-shadow(0 0 10px #FFD43B)!important;stroke:#FFD43B!important;stroke-width:3!important;opacity:1!important}}
.path-context{{opacity:.9!important}} .phase-hidden{{opacity:.08!important;pointer-events:none}}
@media(max-width:1100px){{#viewport{{padding-right:14px}}#sidePanel{{position:relative;right:auto;top:auto;width:auto;max-height:none;margin:10px 14px}}}}
...
#stickyHeader svg{{background:#070A0E;border:1px solid #2B3A48;border-radius:12px 12px 0 0;box-shadow:0 5px 14px rgba(0,0,0,.45)}}
#canvasWrap{{transform-origin:top left;transition:transform .15s ease;display:inline-block}}
svg{{background:#05070A;border:1px solid #2B3A48;border-radius:12px;box-shadow:0 8px 25px rgba(0,0,0,.45)}}
.grid{{stroke:#1D2731;stroke-width:1}} .grid.strong{{stroke:#314151;stroke-width:1.3}}
.flow-line{{fill:none;stroke:#4EA3FF;stroke-width:1.6}} .go-line{{fill:none;stroke:#37D66B;stroke-width:1.8}} .no-line{{fill:none;stroke:#FF4D4F;stroke-width:1.7}}
.parallel-line{{fill:none;stroke:#4EA3FF;stroke-width:1.4}} .parallel-return{{fill:none;stroke:#37D66B;stroke-width:1.45}} .parallel-card,.contract-review-card{{cursor:pointer}}
.parallelGroupTitle{{font-size:11px;font-weight:900;fill:#DDEBFA}} .mergeText{{font-size:8px;font-weight:900;fill:#37D66B}} .parallelDept{{font-size:9px;font-weight:900;fill:#AFC3D5;letter-spacing:.04em}} .parallelTitle{{font-size:10px;font-weight:900;fill:#F4F8FC}} .parallelHint{{font-size:8px;font-weight:800;fill:#8BBDF0}}
.hdr{{font-size:14px;font-weight:900;fill:#F5FAFF}} .num{{font-size:25px;font-weight:900;fill:#4EA3FF}} .stage{{font-size:11px;font-weight:900;fill:#D3E2EE}} .boxtext{{font-size:14px;font-weight:800;fill:#F5FAFF}} .gateText{{font-size:12px;font-weight:900;fill:#F5FAFF}} .goLabel{{font-size:11px;font-weight:900;fill:#37D66B}} .noLabel{{font-size:11px;font-weight:900;fill:#FF4D4F}} .actionTitle{{font-size:9px;font-weight:900;fill:#FF6B6B}} .actionHint{{font-size:7.5px;font-weight:800;fill:#FF9A9A}}
.process-box,.gate,.action-box{{cursor:pointer;transition:filter .15s,stroke-width .15s}} .process-box:hover,.gate:hover,.action-box:hover,.parallel-card:hover,.contract-review-card:hover{{filter:drop-shadow(0 3px 5px rgba(0,0,0,.18));stroke-width:2.4}}
...
#modalCard{{width:min(760px,92vw);max-height:84vh;overflow:auto;background:#101722;border-radius:16px;box-shadow:0 20px 70px rgba(0,0,0,.35);padding:22px}}
#modalCard h2{{margin:0 0 6px;color:#F5FAFF}} .badge{{display:inline-block;padding:5px 9px;border-radius:999px;background:#0B1B2D;color:#155D9B;font-size:11px;font-weight:800}}
.detailGrid{{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:14px}} .detail{{border:1px solid #2D4154;border-radius:10px;padding:10px;background:#0B1118}} .detail b{{display:block;font-size:10px;letter-spacing:.08em;color:#8FB0C8;margin-bottom:5px}} .detail span{{font-size:13px;line-height:1.45;color:#E5EEF5}}
.close{{float:right;border:0;background:#263746;color:#F5FAFF;border-radius:50%;width:32px;height:32px;cursor:pointer;font-size:18px}}
@media(max-width:800px){{.detailGrid{{grid-template-columns:1fr}}}}
</style></head><body>
<div class='toolbar'>
 <span class='title'>STARCOM SOP · Interactive Full Canvas</span>
 <div class='mode-group'>
   <button id='fullModeBtn' class='mode-btn active' type='button'>Full SOP</button>
   <button id='chapterModeBtn' class='mode-btn' type='button'>Chapter</button>
   <button id='focusModeBtn' class='mode-btn' type='button'>Focus</button>
 </div>
 <input id='search' placeholder='Cari proses, dokumen, risiko…'/>
 <select id='dept'><option value='ALL'>Semua departemen</option>{''.join(f'<option>{d}</option>' for d in deps)}</select>
 <select id='chapterSelect' title='Pilih chapter'>
   <option value='ALL'>Semua chapter</option>
   <option value='Opportunity'>Opportunity</option>
   <option value='Solution & Approval'>Solution & Approval</option>
   <option value='Commercial'>Commercial</option>
   <option value='Delivery'>Delivery</option>
   <option value='Acceptance & Cash'>Acceptance & Cash</option>
   <option value='Customer Growth'>Customer Growth</option>
 </select>
 <button id='zoomOutBtn' type='button'>−</button><button id='zoomLabel' type='button'>100%</button><button id='zoomInBtn' type='button'>+</button>
 <button id='readableBtn' type='button'>Readable</button><button id='overviewBtn' type='button'>Overview</button><button id='presentBtn' type='button'>▶ Presentasi</button><button id='resetBtn' type='button'>Reset</button><button id='fullscreenBtn' type='button'>⛶ Full Canvas</button><button id='printBtn' type='button'>Print / PDF</button>
 <div class='phase-nav' id='phaseNav'>
   <button type='button' data-phase='Opportunity'>Opportunity</button>
   <button type='button' data-phase='Solution & Approval'>Solution</button>
   <button type='button' data-phase='Commercial'>Commercial</button>
   <button type='button' data-phase='Delivery'>Delivery</button>
   <button type='button' data-phase='Acceptance & Cash'>Acceptance & Cash</button>
   <button type='button' data-phase='Customer Growth'>After Sales</button>
 </div>
</div>
<div id='viewport'>
...
<rect x='{left}' y='0' width='{width-left-8}' height='104' fill='#05070A'/>
<rect x='{left}' y='6' width='{stage_w-8}' height='54' rx='5' fill='#0B1118' stroke='#30475A'/>
...
<rect x='{left}' y='35' width='{width-left-8}' height='{height-55}' rx='10' fill='#05070A' stroke='#2B3A48'/>
<rect x='{left}' y='40' width='{stage_w-8}' height='54' rx='5' fill='#0B1118' stroke='#30475A'/><text x='{left+(stage_w-8)/2}' y='72' text-anchor='middle' class='hdr'>TAHAP</text>
{''.join(lane_headers)}
{''.join(grid_lines)}{''.join(connectors)}{''.join(rows)}
</svg></div></div>
<aside id='sidePanel'>
  <div class='muted'>PROCESS NAVIGATOR</div>
  <h3 id='panelTitle'>Pilih proses pada diagram</h3>
  <div id='panelOwner' class='muted'>Klik kotak proses untuk fokus dan membaca detail.</div>
  <div id='panelContent'>
    <div class='panel-section'><b>MODE TAMPILAN</b><span>Full SOP untuk seluruh alur, Chapter untuk satu fase, dan Focus untuk satu proses beserta konteksnya.</span></div>
  </div>
  <div class='panel-section'><b>MINI MAP · KLIK UNTUK PINDAH</b><div id='miniMap'></div></div>
</aside>
...
<script>
const processes={p_json};
const departments={dep_json};
const parallelAuthority={parallel_json};
const contractReviewParallel={contract_review_json};
const phases=[
  {{name:'Opportunity',ids:[1,2,3]}},
  {{name:'Solution & Approval',ids:[4,5,6]}},
  {{name:'Commercial',ids:[7,8]}},
  {{name:'Delivery',ids:[9,10,11]}},
  {{name:'Acceptance & Cash',ids:[12,13,14,15]}},
  {{name:'Customer Growth',ids:[16,17,18]}}
];
let zoom=1.12, viewMode='full', selectedProcess=null;
const viewport=document.getElementById('viewport');
const chart=document.getElementById('chart');

function setZoom(v){{
  zoom=Math.max(.55,Math.min(2.2,v));
  document.getElementById('canvasWrap').style.transform=`scale(${{zoom}})`;
  document.getElementById('stickyHeaderWrap').style.transform=`scale(${{zoom}})`;
...
function fitWidth(){{setZoom((viewport.clientWidth-355)/chart.width.baseVal.value)}}
function readableView(){{setZoom(1.25)}}
function resetView(){{
  viewMode='full'; selectedProcess=null; setZoom(1.12);
  document.getElementById('search').value='';
  document.getElementById('dept').value='ALL';
  document.getElementById('chapterSelect').value='ALL';
  clearClasses(); updateModeButtons(); updateSidePanel(null); viewport.scrollTo({{left:0,top:0,behavior:'smooth'}});
}}
function fullscreen(){{const el=document.getElementById('viewport');if(el.requestFullscreen)el.requestFullscreen();}}
function presentationView(){{setZoom(1.32);fullscreen();}}
function clearClasses(){{
  document.querySelectorAll('.process-box,.gate,.action-box,.parallel-card,.contract-review-card').forEach(el=>{{
    el.classList.remove('dim','highlight','focused','path-context','phase-hidden');
  }});
}}
function getPhase(name){{return phases.find(p=>p.name===name)}}
function phaseForId(id){{return phases.find(p=>p.ids.includes(id))}}
function scrollToProcess(id){{
  const el=document.getElementById('process-'+id); if(!el)return;
  const x=parseFloat(el.getAttribute('x'))*zoom; const y=parseFloat(el.getAttribute('y'))*zoom;
  viewport.scrollTo({{left:Math.max(0,x-230),top:Math.max(0,y-145),behavior:'smooth'}});
}}
function showChapter(name){{
  const phase=getPhase(name); if(!phase)return;
  viewMode='chapter'; selectedProcess=null; clearClasses();
  document.querySelectorAll('[data-id]').forEach(el=>{{if(!phase.ids.includes(+el.dataset.id))el.classList.add('phase-hidden')}});
  document.getElementById('chapterSelect').value=name; updateModeButtons(); updateSidePanel(null,phase);
  scrollToProcess(phase.ids[0]);
}}
function focusProcess(id){{
  const p=processes.find(x=>x.id===id); if(!p)return;
  selectedProcess=id; viewMode='focus'; clearClasses();
  const phase=phaseForId(id);
  document.querySelectorAll('[data-id]').forEach(el=>{{
    const eid=+el.dataset.id;
    if(eid===id) el.classList.add('focused');
    else if(phase && phase.ids.includes(eid)) el.classList.add('path-context');
    else el.classList.add('dim');
  }});
  updateModeButtons(); updateSidePanel(p); updateMiniMap(); scrollToProcess(id);
}}
function setFullMode(){{viewMode='full';selectedProcess=null;clearClasses();document.getElementById('chapterSelect').value='ALL';updateModeButtons();updateSidePanel(null);updateMiniMap();}}
function updateModeButtons(){{
  document.getElementById('fullModeBtn').classList.toggle('active',viewMode==='full');
  document.getElementById('chapterModeBtn').classList.toggle('active',viewMode==='chapter');
  document.getElementById('focusModeBtn').classList.toggle('active',viewMode==='focus');
}}
function applyFilters(){{
  const q=document.getElementById('search').value.toLowerCase();
  const dep=document.getElementById('dept').value;
  if(viewMode!=='full') clearClasses();
  document.querySelectorAll('.process-box,.gate,.action-box').forEach(el=>{{
    const id=+el.dataset.id; const p=processes.find(x=>x.id===id);
    const okQ=!q||JSON.stringify(p).toLowerCase().includes(q); const okD=dep==='ALL'||p.owner===dep;
    el.classList.toggle('dim',!(okQ&&okD)); el.classList.toggle('highlight',okQ&&q.length>1);
  }});
  if(q.length>1){{const first=processes.find(p=>JSON.stringify(p).toLowerCase().includes(q)&&(dep==='ALL'||p.owner===dep));if(first)scrollToProcess(first.id)}}
}}
function updateSidePanel(p,phase=null){{
  const title=document.getElementById('panelTitle'); const owner=document.getElementById('panelOwner'); const body=document.getElementById('panelContent');
  if(phase){{
    title.textContent=phase.name; owner.textContent=`${{phase.ids.length}} proses dalam chapter ini`;
    const names=phase.ids.map(id=>processes.find(p=>p.id===id).name).join(' → ');
    body.innerHTML=`<div class='panel-section'><b>ALUR CHAPTER</b><span>${{names}}</span></div><div class='panel-section'><b>CARA MEMBACA</b><span>Klik salah satu proses pada diagram atau mini map untuk membuka Focus Mode tanpa mengecilkan teks.</span></div>`;
    return;
  }}
  if(!p){{title.textContent='Pilih proses pada diagram';owner.textContent='Klik kotak proses untuk fokus dan membaca detail.';body.innerHTML=`<div class='panel-section'><b>MODE TAMPILAN</b><span>Full SOP untuk seluruh alur, Chapter untuk satu fase, dan Focus untuk satu proses beserta konteksnya.</span></div>`;return;}}
  title.textContent=`${{p.id}}. ${{p.name}}`; owner.textContent=`Owner: ${{p.owner}} · SLA: ${{p.sla}}`;
  body.innerHTML=`
    <div class='panel-section'><b>KEGIATAN UTAMA</b><span>${{p.activity}}</span></div>
    <div class='panel-section'><b>OUTPUT</b><span>${{p.output}}</span></div>
    <div class='panel-section'><b>DECISION GATE</b><span>${{p.gate}}</span></div>
    <div class='panel-section'><b>GO</b><span>${{p.go}}</span></div>
    <div class='panel-section'><b>NO-GO / TINDAKAN</b><span>${{p.nogo}} — ${{p.solution}}</span></div>
    <div class='panel-section'><b>DOKUMEN</b><span>${{p.documents}}</span></div>
    <div class='panel-section'><b>KPI & RISIKO</b><span>${{p.kpi}}<br><br>${{p.risk}}</span></div>`;
}}
function updateMiniMap(){{
  const mm=document.getElementById('miniMap');
  mm.innerHTML=processes.map(p=>{{const c=departments[p.owner];return `<div class='mini-item ${{selectedProcess===p.id?'active':''}}' data-mini-id='${{p.id}}'><span class='mini-dot' style='background:${{c.color}}'></span><span>${{p.id}}. ${{p.name}}</span></div>`}}).join('');
  mm.querySelectorAll('.mini-item').forEach(el=>el.addEventListener('click',()=>focusProcess(+el.dataset.miniId)));
}}
function showDetail(id){{focusProcess(id)}}
...
function showContractReviewDetail(index){{const item=contractReviewParallel[index];const c=departments[item.department];document.getElementById('modalBody').innerHTML=`<span class='badge' style='background:${{c.soft}};color:${{c.color}}'>${{item.department}}</span><h2>${{item.title}}</h2><div class='detailGrid'><div class='detail'><b>REVIEW SCOPE</b><span>${{item.detail}}</span></div><div class='detail'><b>OUTPUT</b><span>${{item.output}}</span></div><div class='detail'><b>POLA KERJA</b><span>Dilaksanakan paralel oleh Legal dan Finance setelah Negotiation dinyatakan deal.</span></div><div class='detail'><b>ALUR BERIKUTNYA</b><span>Hasil Legal dan Finance digabung pada consolidation/merge gate, kemudian dilanjutkan ke Kick Off Internal.</span></div></div>`;document.getElementById('modal').style.display='flex';}}
...
document.getElementById('dept').addEventListener('change',applyFilters);
document.getElementById('chapterSelect').addEventListener('change',e=>e.target.value==='ALL'?setFullMode():showChapter(e.target.value));
document.getElementById('fullModeBtn').addEventListener('click',setFullMode);
document.getElementById('chapterModeBtn').addEventListener('click',()=>showChapter(document.getElementById('chapterSelect').value==='ALL'?'Opportunity':document.getElementById('chapterSelect').value));
document.getElementById('focusModeBtn').addEventListener('click',()=>selectedProcess?focusProcess(selectedProcess):focusProcess(1));
document.getElementById('zoomOutBtn').addEventListener('click',zoomOut);
document.getElementById('zoomInBtn').addEventListener('click',zoomIn);
document.getElementById('zoomLabel').addEventListener('click',actualSize);
document.getElementById('readableBtn').addEventListener('click',readableView);
document.getElementById('overviewBtn').addEventListener('click',fitWidth);
document.getElementById('presentBtn').addEventListener('click',presentationView);
document.getElementById('resetBtn').addEventListener('click',resetView);
document.getElementById('fullscreenBtn').addEventListener('click',fullscreen);
document.getElementById('printBtn').addEventListener('click',()=>window.print());
document.querySelectorAll('#phaseNav button').forEach(btn=>btn.addEventListener('click',()=>showChapter(btn.dataset.phase)));
updateMiniMap(); setZoom(1.12); updateModeButtons();
